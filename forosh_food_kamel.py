...
async def charge_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if data == "charge_custom":
        message = (
            "💳 مبلغی را که می‌خواهید شارژ کنید(هرچقدر مد نظرتونه) برای شماره کارت زیر واریز کنید و رسید را به پیوی ادمین ارسال کنید ✅\n\n"
...
# If you still need a separate handler for the custom amount (not receiving any numeric input),
# you can remove any numeric validation from it as shown below.
