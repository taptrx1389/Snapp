...
def save_user_data():
    data = {
        ...
        "USER_RECENT_PURCHASES": USER_RECENT_PURCHASES
    }
...
def load_user_data():
    ...
    try:
        with open(DATA_FILE, "r", encoding="utf-8") as f:
            ...
            USER_CHARGED = data.get("USER_CHARGED", {})
            USER_PURCHASED = data.get("USER_PURCHASED", {})
            USER_RECENT_PURCHASES = data.get("USER_RECENT_PURCHASES", {})
    except FileNotFoundError:
        USER_BALANCES = {}
        USER_CHARGED = {}
        USER_PURCHASED = {}
        USER_RECENT_PURCHASES = {}
...
async def buy_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Save current user data persistently
    save_user_data()
...
async def charge_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if data == "charge_custom":
        message = (
            "💳 مبلغی را که می‌خواهید شارژ کنید(هرچقدر مد نظرتونه) برای شماره کارت زیر واریز کنید و رسید را به پیوی ا[...]\n"
        ...
        await query.edit_message_text(message, reply_markup=support_keyboard)
        ...
        try:
            ...
            return ConversationHandler.END
        message = (
            f"💳 مبلغ {amount} را برای شماره کارت زیر واریز کنید و رسید را به پیوی ادمین ارسال کنید ✅\n\n"
...
async def charge_custom_amount(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    message = (
        "💳 مبلغی را که می‌خواهید شارژ کنید(هرچقدر مد نظرتونه) برای شماره کارت زیر واریز کنید و رسید را به پیوی ادم[...]\n"
        "💳 شماره کارت : 6037998233895712\n\n"
        "👤بنام پویان شیرازی"
    )
    support_keyboard = InlineKeyboardMarkup([
    ...
    await update.message.reply_text(message, reply_markup=support_keyboard)
    return ConversationHandler.END
...
# =====================================================================
async def main():
    # Load persistent user data at startup.
    load_user_data()
    application = Application.builder().token("7039579736:AAFmD5CePJj47IESG157aG7UxaJVQGcLXEk").build()
